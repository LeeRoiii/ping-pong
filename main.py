...
def initialize_game():
    ...
    ball_y = 100
    ball_radius = 20
    ball_speed_x = 5  # Increase the horizontal speed
    ball_speed_y = 5  # Increase the vertical speed
...
def main():
    ...
    cv2.namedWindow("Volleyball Game", cv2.WND_PROP_FULLSCREEN)
    cv2.setWindowProperty("Volleyball Game", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
    ...
    while True:
        ...
        if not game_over:
            ...
            if paddle_x < ball_x < paddle_x + paddle_width and paddle_y < ball_y < paddle_y + paddle_height:
                ball_speed_y = -ball_speed_y
                score += 1
                ball_speed_x += 1  # Increase ball speed when the player scores
            ...
            cv2.putText(frame, "Game Over! Press 'R' to Try Again.", (screen_width // 4, screen_height // 2), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        ...
        # Show the frame
        cv2.imshow("Volleyball Game", frame)
